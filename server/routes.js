...
router.get("/module/:id", (req, res) => {
  res.sendFile(path.join(__dirname, "../src", "pages", "module.html"), (err) => {
    if (err) {
      res.status(err.status || 500).send("Internal Server Error");
    }
  });
...
router.get("/storage/:module/:id/:name", (req, res) => {
  const { module, id, name } = req.params;
  res.sendFile(path.join(__dirname, "../storage", module, id, name), (err) => {
    if (err) {
      res.status(err.status || 500).send("Internal Server Error");
    }
  });
